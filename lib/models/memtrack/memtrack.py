...
from lib.models.memtrack.vit_ce import vit_large_patch16_224_ce, vit_base_patch16_224_ce
...
class MemTrack(nn.Module):
    ...
    def forward(self, template:torch.Tensor,    
                ...
                ce_keep_rate=None,
                template_lable=None,
                template_pixle_lable=None,
                search_lable=None,
                state='one_stage_train'):
        ...
        """
        N,B= search.shape[0],search.shape[1]
        assert N == 5
        self.bs = B
        search = rearrange(search,'n b c h w -> (n b) c h w')
        template = template.unsqueeze(0).repeat(N,1,1,1,1)
        template = rearrange(template,'n b c h w -> (n b) c h w')
        template_pixle_lable = template_pixle_lable.unsqueeze(0).repeat(N,1,1,1,1)
        template_pixle_lable = rearrange(template_pixle_lable,'n b c h w -> (n b) c h w') 
        
        # getting features on backbone
        z,x, aux_dict = self.backbone(z=template, x=search,
                                    ce_template_mask=ce_template_mask,
                                    ce_keep_rate=ce_keep_rate,template_pixle_lable=template_pixle_lable,
                                    return_last_attn=False)
 
        z = rearrange(z,'(n b) (h w) c -> n b c h w',n=N,h=int(z.shape[1]**.5))[0]
        x = rearrange(x,'(n b) (h w) c -> n b c h w',n=N,h=int(x.shape[1]**.5))
        ...
        for i in range(N):
            if i == 0:
                x_feat = self.memory_network(mode='initialize',z_feat=z,mask=template_lable,x_feat=x[i],x_pos=x_pos)
                x_feat_list.append(x_feat)
                
            else:
                memory = self.memory_network(mode='encode',feat=x[i-1],mask=search_lable[i-1])
                self.memory_network(mode='addMemory',score_ve=None,in_memorys=memory,state='train')
                x_feat = self.memory_network(mode='decode',x_feat=x[i],x_pos=x_pos)
                x_feat_list.append(x_feat)
        ...
        return self.adjust_outdict(out)
    ...
    def visiual_mask_imgs(self,img,label,mask_img):
        sv_path = '/home/zhoujiawei/my_model/ARTrack_1/vis_mask_imgs/'
...
def build_memtrack(cfg, training=True):
    ...
    model = MemTrack(backbone,box_head,memory_network,seek_tgt,
    ...
    if ('MemTrackSeq' in cfg.MODEL.PRETRAIN_FILE or 'DropTrack' in cfg.MODEL.PRETRAIN_FILE)and training:
        pretrained_path = os.path.join(current_dir, '../../../pretrained_models')
        pretrained = os.path.join(pretrained_path, cfg.MODEL.PRETRAIN_FILE)
