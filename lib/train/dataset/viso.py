...
class VISO(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=opencv_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        root = env_settings().viso if root is None else root
        super().__init__('VISO', root, image_loader)

        with open(os.path.join(root, 'VISO.json'), 'r') as f:
    ...
    def _read_bb_anno(self, seq_id):
        ...
        gt = torch.from_numpy(np.array(self.meta_data[seq_name]['gt_rect'])).to(torch.float32)
    ...
    def _get_frame_path(self, seq_id, frame_id):
        seq_name = self.sequence_list[seq_id]
        img_path = self.meta_data[seq_name]['img_names'][frame_id]
        return os.path.join(self.root,img_path)    # frames start from 1
...
if __name__ =='__main__':
    root = '/home/zhoujiawei/satellite_video_datasets/VISO/SOT/sot'
    base_path = '/home/zhoujiawei/my_model/test_viso'
    dataset = VISO(root=root,split='train')
    num = 0
    for i in range(len(dataset.sequence_list)):
        ...
        if len(invisible_id[0]):
            index = list(set(list(np.array([invisible_id[0][0]-1]+ list(invisible_id[0]) +[invisible_id[0][-1]+1]).clip(0,len(info['bbox'])))))
