...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if output_sz is not None:
        ...
        im_crop_padded = cv.resize(im_crop_padded.astype(np.uint8), (output_sz, output_sz))
...
def transform_image_to_crop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
    ...
    # get center of memory state in previous frame
    box_in_center = box_in[0:2] + 0.5 * box_in[2:4]
    # if camera is stationary, this is reasonable
...
def transform_image_to_bacthcrop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
                            crop_sz: torch.Tensor, normalize=False) -> torch.Tensor:
    """ Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image
    args:
        box_in - the box for which the co-ordinates are to be transformed
    ...
    box_out = torch.cat((box_out_center - 0.5 * box_out_wh, box_out_wh),dim=1)
    if normalize:
        return box_out / crop_sz[0]
    else:
        return box_out

def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None,history_bbox=None):
    ...
    if history_bbox is not None:
        history_bbox = [transform_image_to_bacthcrop(a_gt,a_ex,rf,crop_sz,normalize=True)
                        for a_gt, a_ex, rf in zip(history_bbox, box_extract, resize_factors) ]
        return frames_crop, box_crop, att_mask, masks_crop,history_bbox
