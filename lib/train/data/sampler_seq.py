...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5,pre_num=24,template_gap=24,reverse_prob=0.5):
        """
        args:
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
                            allow_invisible=False, force_invisible=False,state=None,order=None):
        ...
        if state == 'search':
            if len(valid_ids) == num_ids:
                ...
                if order:
                    min_index = random.choices(range(len(valid_ids)-num_ids), k=1)[0]
                    ids = valid_ids[min_index:min_index+num_ids]
                else:
                    max_index = random.choices(range(len(valid_ids))[num_ids:], k=1)[0]
                    ids = valid_ids[max_index-num_ids:max_index]
        else:
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    # Sample test and train frames in a causal manner, i.e. search_frame_ids > template_frame_ids
                        while search_frame_ids is None:
                            ...
                            # prev_frame_ids,_ = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,
                            #                                           min_id=base_frame_id[0] - self.template_gap - gap_increase,
                            ...
                            search_frame_ids = self._sample_visible_ids(visible, min_id=template_frame_ids[0] + 1,
                                                                    ...
                                                                    num_ids=self.num_search_frames,state='search',order=order_sample)
                ...
                # except:
                #     raise ValueError(seq_id,'sample get error')
            else:
            ...
            #     static_prob = 0.5
            #     static = False
            #     if order_sample:   # sample history bbox in order
            #         if random.random() < static_prob:   # whether static 
            #             static = True
            #         history_ids = self.sample_history_ids(valid_ids,search_frame_ids,mode='order',static=static)

            #     else:   # sample history bbox in reversed
            #         if random.random() < static_prob:   # whether static 
            #             static = True
            #         search_frame_ids = list(reversed(search_frame_ids))
            #         history_ids = self.sample_history_ids(valid_ids,search_frame_ids,mode='reversed',static=static)
            #         for i,ids in enumerate(history_ids):
            #             history_ids[i] = list(reversed(ids))
                    
            #     assert history_ids is not None
            # except Exception as e:
            #     print(e)
            try:
                # history_bbox = [seq_info_dict['bbox'][i] for i in history_ids]
                # history_bbox = torch.stack(history_bbox,dim=0).reshape(-1,4)
                ...
                # history_bbox = self.batch_clip_bbox(history_bbox,H,W)
                ...
                # draw_shbbox(search_frames,search_anno['bbox'])
                # draw_tmbbox(template_frames,template_anno['bbox'])
                # draw_hisbbox([search_frames[0]],history_bbox)
                # cv2.imwrite('t1.jpg',search_frames[0])
