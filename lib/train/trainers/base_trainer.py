...
class BaseTrainer:
    ...
    def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False,custom_ckpt=None):
        ...
        for i in range(num_tries):
            try:
                if load_latest:
                    self.load_checkpoint(checkpoint=custom_ckpt)
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    # if epoch > (max_epochs - 10) or save_every_epoch or epoch % 100 == 0:
                        ...
                        #         extrect_module = importlib.import_module('tracking.test')
                        #         expr_func = getattr(extrect_module, 'run_tracker')
                        #         expr_func(tracker_name=self.settings.script_name,tracker_param=self.settings.config_name,
                        #                 dataset_name='satsot',threads=40,num_gpus=3,test_epoch=epoch,train=True)
                        #         print('*'*20,'start analysing','*'*20)
                        #         extrect_module = importlib.import_module('tracking.analysis_results')
                        #         expr_func = getattr(extrect_module, 'analysis')
                        #         expr_func(self.settings.script_name,self.settings.config_name)
            except:
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        print('resume checkpoint path:',checkpoint_path)
