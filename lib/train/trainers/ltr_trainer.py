...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader,1):
            ...
            tb_idx = i + (self.epoch-1) * loader.__len__() # 多少个 batch
            ...
            if self.wandb_writer is not None and i % self.settings.print_interval == 0:
               if self.settings.local_rank in [-1, 0]:
                   self.wandb_writer.write_log(self.stats, self.epoch,data)
    ...
    def apply_colormap_to_image(self,gray_image):
        # Apply colormap (e.g., 'viridis', 'plasma', 'inferno', etc.)
        colormap = plt.get_cmap('viridis')  # Choose a colormap
        colored_image = colormap(gray_image)  # Apply colormap
