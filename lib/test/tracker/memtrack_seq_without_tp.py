...
class MemTrackSeq(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # motion constrain
        self.output_window = hann2d(torch.tensor([self.cfg.TEST.SEARCH_SIZE, self.cfg.TEST.SEARCH_SIZE]).long(), centered=True).cuda()
    ...
    def initialize(self, image, info: dict):
        ...
        # d_patch_arr, d_resize_factor, d_amask_arr = sample_target(image, info['init_bbox'], self.params.search_factor,
        #                                             output_sz=self.params.search_size)#output_sz=self.params.template_size
        ...
        # search_bbox_cxcy = torch.tensor((self.params.search_size/2,self.params.search_size/2))
        # search_bbox = torch.cat([search_bbox_cxcy,search_bbox_wh],dim=0)
    ...
    def track(self, image, info: dict = None):
        ...
        heatmap_x = out_dict['heatmap'].mean(dim=1,keepdim=True) * (-1)
        heatmap_x = torch.exp(heatmap_x*10)
        heatmap_x = F.interpolate(heatmap_x,self.params.search_size,mode='bilinear',align_corners=True)
        heatmap_x = heatmap_x - heatmap_x.mean()
        heatmap_x = heatmap_x.clamp(0,heatmap_x.max())
        heatmap_x = (heatmap_x - heatmap_x.min()) / (heatmap_x.max() - heatmap_x.min())
        heatmap_x = heatmap_x.reshape(self.params.search_size,-1).cpu().detach().numpy()
        ...
        # pred_score_map = pred_score_map - pred_score_map.min() / pred_score_map.max() - pred_score_map.min()
        response = pred_score_map * self.output_window
        # response = response - response.min() / response.max() - response.min()
        # if self.frame_id == 190:
        #     print('check')
        # update memory 
        ...
        # if self.frame_id > 40 and not use_network: 
        #     x = self.trajectory[:,0][-40:]
        #     y = self.trajectory[:,1][-40:]
        #     frame_num = np.arange(0,40)
        #     func_x = np.polyfit(frame_num, x, 1)
        #     func_y = np.polyfit(frame_num, y, 1)
        #     pred_x = np.polyval(func_x,40 + 1)
        #     pred_y = np.polyval(func_y,40 + 1)
        ...
        #     # else:
        #         # v_x = (self.trajectory[-1,0] - self.trajectory[-10,0]) / 10
        #         # v_y = (self.trajectory[-1,1] - self.trajectory[-10,1]) / 10
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                # plt.imshow(x_patch_arr, alpha=1)
                # plt.imshow(heatmap_x, cmap='jet', alpha=0.1)
                # plt.colorbar()
                # plt.savefig(os.path.join(self.save_dir, "%04dheatmap_x.jpg" % frame_id))
                # plt.close()
                heatmap_x =  np.uint8(255 * heatmap_x)
                heatmap_x_colored_heatmap = cv2.applyColorMap(heatmap_x, cv2.COLORMAP_JET)
                show_heatmap_x = cv2.addWeighted(heatmap_x_colored_heatmap, 0.4,x_patch_arr, 1, 0)
                cv2.imwrite(os.path.join(self.save_dir, "%04dheatmap_x.jpg" % frame_id),show_heatmap_x)
        ...
        if w <= h:
            ...
            # w = math.exp(ratio-1) * w
            h = (math.log10(1/ratio) * 2 + 1) * h
        # w,h = self.state[2] * resize_factor, self.state[3]*resize_factor
    ...
    def determine_updatemMemory(self,pred_label,resize_factor,heatmap,response):
        ...
        # response = response.clone().cpu().squeeze()
        ...
        # response_area = (response > response.max()-0.1).sum()
        # # * (math.log10(max(self.store_wh[:,0].mean(),self.store_wh[:,1].mean()) / min(self.store_wh[:,0].mean(),self.store_wh[:,1].mean())) * 2 + 1)
        # response_area_threshold = prev_area * (0.4)**2 * (math.log10(max(self.store_wh[:,0].mean(),self.store_wh[:,1].mean()) / min(self.store_wh[:,0].mean(),self.store_wh[:,1].mean())) * 2 + 1)
        # isin_area = response_area < response_area_threshold
        
        high_val_center = self.peak_num(heatmap)
        # high_val_center_response = self.peak_num(response.numpy())
        if len(high_val_center) == 1:
            heatmap_cx = high_val_center[0][1]
            heatmap_cy = high_val_center[0][0]
            # response_cx = high_val_center_response[0][1]
            # response_cy = high_val_center_response[0][0]
        else:
            return False,False
        if prev_w <= prev_h:
            ...
            heatmap_isin_w = abs(heatmap_cx - prev_cx) / resize_factor< self.store_wh[:,0].mean()
            heatmap_isin_h = abs(heatmap_cy - prev_cy) / resize_factor< threshold_h
            ...
            heatmap_isin_w = abs(heatmap_cx - prev_cx) / resize_factor< threshold_w
            heatmap_isin_h = abs(heatmap_cy - prev_cy) / resize_factor< self.store_wh[:,1].mean()
        ...
        #     return False,True
        if (pred_label_area > 0.7 *prev_area and pred_label_area < 1.2*prev_area) and (isin_w and isin_h) and (heatmap_isin_w and heatmap_isin_h) and (response_isin_w and response_isin_h):
            return True,True
        elif (response_isin_w and response_isin_h): # (response_isin_w and response_isin_h) and 
            return False,True
        # if (pred_label_area > 0.7 *prev_area and pred_label_area < 1.2*prev_area) and (isin_w and isin_h) and (heatmap_isin_w and heatmap_isin_h) and (response_isin_w and response_isin_h):
        #     return True,True
        # elif (response_isin_w and response_isin_h):
        #     return False,True
        # elif (heatmap_isin_w and heatmap_isin_h) and (response_isin_w and response_isin_h):
        #     return False,True
        else:
            return False,False
