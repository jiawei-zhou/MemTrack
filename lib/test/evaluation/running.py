...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if not os.path.exists(os.path.abspath(os.path.join(base_results_path,'..'))):
        os.makedirs(os.path.abspath(os.path.join(base_results_path,'..')))
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8,train=False,gpu_id=None):
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k','SV248S','SatSOT','OOTB','VISO','lasot','otb','uav']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
                if os.path.isfile(bbox_file):
                    try:
                        len_pred = len(np.loadtxt(bbox_file,delimiter=',',dtype=np.float32))
                    except:
                        len_pred = len(np.loadtxt(bbox_file,delimiter='\t',dtype=np.float32))
                    return len_pred == len(seq.ground_truth_rect)
                else:
                    return False
            else:
    ...
    if _results_exist() and not debug and not train:
        print('FPS: {}'.format(-1))
        return
...
def gpu_task_scheduler(gpu_ids, num_gpus):
    """分配 GPU ID 的生成器"""
    while True:
        for gpu_id in gpu_ids:
            yield gpu_id

def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8,train=False):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug,train=train)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, num_gpus,train) for seq, tracker_info in product(dataset, trackers)]
        with multiprocessing.Pool(processes=threads) as pool:
            pool.starmap(run_sequence, param_list)
    end_time = time.time()
    print(f'consume times:{ (end_time-start_time) / 60} min')
