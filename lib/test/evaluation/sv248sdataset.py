...
import json  
import os
class SV248SDataset(BaseDataset):
    """ OTB-2015 dataset
    Publication:
    ...
    def __init__(self): # self由BaseDatset传入
        super().__init__()
        self.base_path = self.env_settings.sv248s_path     # 数据集地址
        with open(os.path.join(self.base_path, 'SV248S.json'), 'r') as f:
    ...
    def _construct_sequence(self, name):
        ...
        return Sequence(name,frame,'SV248S',ground_truth)
