...
class SatSOTDataset(BaseDataset):
    ...
    def __init__(self): # self由BaseDatset传入
        super().__init__()
        self.base_path = self.env_settings.satsot_path     # 数据集地址
        with open(os.path.join(self.base_path, 'SatSOT.json'), 'r') as f:
    ...
    def _construct_sequence(self, name):
        ...
        video_dir,attr,init_rect, gt_rect, img_names  = sub_dict[0], sub_dict[1], sub_dict[2], sub_dict[3], sub_dict[4]
        ...
        return Sequence(name,frame,'SatSOT',ground_truth)
