...
def evaluate_vit(model, template, search,target_label,ce_template_mask,ce_keep_rate):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=('profile',1,template,search,target_label,ce_template_mask,ce_keep_rate),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(mode='profile',frame_id=i+1,template=template, search=search,template_pixle_label=target_label,ce_template_mask=ce_template_mask,ce_keep_rate=ce_keep_rate)
        start = time.time()
        for i in range(T_t):
            _ = model(mode='profile',frame_id=i+1,template=template, search=search,template_pixle_label=target_label,ce_template_mask=ce_template_mask,ce_keep_rate=ce_keep_rate)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "memtrack_seq":
        ...
        box_mask_z = torch.zeros(1,144)
        box_mask_z[0,65] = 1
        box_mask_z = box_mask_z.to(torch.bool).to(device)
        sz = z_sz//cfg.MODEL.BACKBONE.STRIDE
        target_label = torch.ones(1,1,z_sz,z_sz).to(torch.float32)
        target_label = F.interpolate(target_label,(sz,sz)).to(device)
        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_vit(model, template, search, target_label,box_mask_z,None)
        else:
            evaluate_vit_separate(model, template, search, target_label,box_mask_z,None)
